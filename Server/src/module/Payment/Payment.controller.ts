...
export const createOrder = async (req: Request, res: Response) => {
    try {
        const { userId, tourId, amount, currency = 'INR', receipt } = req.body;
    ...
    try {
        ...
        if (generated_signature === signature) {
            const payment = await PaymentModel.findOneAndUpdate(
                ...
                { razorpayPaymentId: paymentId, razorpaySignature: signature, status: "Completed" },
                { new: true }
            );
            if (!payment) {
        ...
        return res.status(500).json({ success: false, message: "Internal Server Error", error: error || "Internal Server Error" });
    }
};
...
export const Refund = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ success: false, message: error || "Refund failed" });
    }
};
